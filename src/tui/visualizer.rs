/// visualizer.rs — The legacy terminal Visualizer trait and its data types.
///
/// Vendored unmodified from audio-viz-tui/src/visualizer.rs except for the
/// `visualizer_utils` import path in spec_to_bars. Installed TUI visualizers
/// (src/visualizers/tui/) implement this trait; src/term/ adapts it to the
/// windowed app's `crate::visualizer::Visualizer`.
...
pub fn spec_to_bars(fft: &[f32], lo_bins: &[usize], hi_bins: &[usize]) -> Vec<f32> {
    lo_bins
        ...
        .map(|(&lo, &hi)| {
            ...
            let rms = crate::tui::visualizer_utils::rms(slice);
